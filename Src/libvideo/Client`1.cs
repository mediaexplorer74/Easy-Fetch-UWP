...
namespace VideoLibrary
{
  public class Client<T> : IService<T>, IAsyncService<T>, IDisposable where T : Video
  {
    ...
    {
      return this.baseService.GetVideo(videoUri, new Func<string, Task<string>>(this.SourceFactory));
    }

    public IEnumerable<T> GetAllVideos(string videoUri)
    {
      return this.baseService.GetAllVideos(videoUri, new Func<string, Task<string>>(this.SourceFactory));
    }

    public Task<T> GetVideoAsync(string videoUri)
    {
      return this.baseService.GetVideoAsync(videoUri, new Func<string, Task<string>>(this.SourceFactory));
    }

    public Task<IEnumerable<T>> GetAllVideosAsync(string videoUri)
    {
      return this.baseService.GetAllVideosAsync(videoUri, new Func<string, Task<string>>(this.SourceFactory));
    }
  }
}
