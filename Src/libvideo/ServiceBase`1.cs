...
namespace VideoLibrary
{
  public abstract class ServiceBase<T> : IService<T>, IAsyncService<T> where T : Video
  {
    ...
    internal T GetVideo(string videoUri, Func<string, Task<string>> sourceFactory)
    {
      return this.GetVideoAsync(videoUri, sourceFactory).GetAwaiter().GetResult();
    }

    public IEnumerable<T> GetAllVideos(string videoUri)
    {
      return this.GetAllVideosAsync(videoUri).GetAwaiter().GetResult();
    }

    internal IEnumerable<T> GetAllVideos(string videoUri, Func<string, Task<string>> sourceFactory)
    {
      return this.GetAllVideosAsync(videoUri, sourceFactory).GetAwaiter().GetResult();
    }
