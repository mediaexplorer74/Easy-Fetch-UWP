  ...
  public class YouTube : ServiceBase<YouTubeVideo>
  {
      ...
      Func<string, Task<string>> sourceFactory)
    {
        if (!this.TryNormalize(videoUri, out videoUri))
        {
            ...
            System.Diagnostics.Debug.WriteLine("URL is not a valid YouTube URL!");
            return default;
        }
    ...
    private bool TryNormalize(string videoUri, out string normalized)
    {
      ...
      videoUri = new StringBuilder(videoUri)
    ...
    private IEnumerable<YouTubeVideo> ParseVideos(string source)
    {
      ...
      if (jsPlayer != null)
      {
        ...
        if (string.Equals(jtoken1 != null ? jtoken1.Value<string>() : (string) null, "error", StringComparison.OrdinalIgnoreCase))
        ...
        string errorReason = jtoken2 != null ? jtoken2.Value<string>() : (string) null;
        ...
        bool isLiveStream = jtoken3 != null && jtoken3.Value<bool>();
        JToken jtoken4 = playerResponseJson.SelectToken("videoDetails.title");
        string title = jtoken4 != null ? jtoken4.Value<string>() : (string) null;
        JToken jtoken5 = playerResponseJson.SelectToken("videoDetails.lengthSeconds");
        int? second = jtoken5 != null ? new int?(jtoken5.Value<int>()) : new int?();
        JToken jtoken6 = playerResponseJson.SelectToken("videoDetails.author");
        string author = jtoken6 != null ? jtoken6.Value<string>() : (string) null;
        ...
        else
        {
          ...
          if (streamFormat != null)
            streamObjects.AddRange((IEnumerable<JToken>) streamFormat.ToArray<JToken>());
          JToken streamAdaptiveFormats = playerResponseJson.SelectToken("streamingData.adaptiveFormats");
          if (streamAdaptiveFormats != null)
            streamObjects.AddRange((IEnumerable<JToken>) streamAdaptiveFormats.ToArray<JToken>());
          foreach (JToken jtoken7 in streamObjects)
          {
            JToken item = jtoken7;
            JToken jtoken8 = item.SelectToken("url");
            string urlValue = jtoken8 != null ? jtoken8.Value<string>() : (string) null;
            ...
            {
              string cipherValue = ((item.SelectToken("cipher") ?? item.SelectToken("signatureCipher")) ?? (JToken) string.Empty).Value<string>();
    ...
    {
      return string.IsNullOrWhiteSpace(YouTube._signatureKey) ? "signature" : YouTube._signatureKey;
    }
  }
}
