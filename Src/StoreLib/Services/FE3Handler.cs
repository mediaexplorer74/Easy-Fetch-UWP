...
namespace StoreLib.Services
{
    public static class FE3Handler
    {
        ...
        /// <returns></returns>
        public static async Task<string> SyncUpdatesAsync(string WuCategoryID, CancellationToken cancellationToken)
        {
        ...
        public static void ProcessUpdateIDs(string Xml, out IList<string> RevisionIDs, out IList<string> UpdateIDs, out IList<PackageItem> Packages)
        {
            XmlDocument doc = new XmlDocument();
            doc.LoadXml(Xml);
            ...
            foreach (XmlNode node in nodes)
            {
                ...
                packageItem.PackageMoniker = packageName;
                packageItem.PackageType = packageType;
                packageItem.IsAppxBundle = isAppxBundle;
                packageItem.IsAppxFramework = isAppxFramework;
        ...
        public static void ProcessMainPackageUpdateIDs(string Xml, out IList<string> RevisionIDs, out IList<string> PackageNames, out IList<string> UpdateIDs)
        {
            ...
            IEnumerable<XElement> updateInfos = doc.Root.Descendants()
                .Where(e => e.Name.LocalName == "UpdateInfo" && mainInstallIds.Contains(e.Elements().First(e1 => e1.Name.LocalName == "ID").Value));
